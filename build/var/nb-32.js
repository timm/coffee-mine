(function() {
  var Bins, Col, Distribution, Learner, Nb, Normal, Num, R, Rand, Sym, e, errorWrapper, fs, getFileAsLines, ifFileExists, inf, ninf, pi, pow, show, shuffle, sqrt;
  ...
  Nb = (function() {
    ...
    Nb.prototype.about = function(cells) {
      return cells[this.theKlass.pos];
    };
    Nb.prototype.test = function(cells) {
      var best, counts, h, klass, like, nKlasses, prior, value, what, _i, _len, _ref, _ref2;
      what = null;
      best = -1;
      ...
      for (klass in _ref) {
        counts = _ref[klass];
        prior = counts.n / (this.rows + this.k * nKlasses);
        like = prior;
        _ref2 = this.head;
        for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
          ...
          if (!h.klassp) {
            if (!this.missingp((value = cells[h.pos]))) {
              like *= h.prob(value, klass, this.m, prior);
            }
          }
        }
        if (like > best) {
    ...
    Nb.prototype.readData = function(row, cells) {
