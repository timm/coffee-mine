(function() {
  ...
  Nb = (function() {
    ...
    Nb.prototype.readData = function(row, cells) {
      var d, h, klass, klassId, value, _i, _len, _ref, _results;
      if (cells.length >= this.head.length) {
        klassId = this.theKlass.pos;
        klass = cells[klassId];
        this.theKlass.add(klass, klassId);
        show(this.theKlass.f);
        d = this.theKlass.f[klass];
        show("klassId " + klassId + " klass " + klass + " d " + d);
        _ref = this.head;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          h = _ref[_i];
          _results.push(!this.missingp((value = cells[h.pos])) ? (value = h.prep(value), h.add(value, d)) : void 0);
        }
        return _results;
      }
    };
